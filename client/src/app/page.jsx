...
const Dashboard = () => {
  ...
  const [cardStats, setCardStats] = useState(null); // Stats riêng cho 4 summary cards (không có date filter)
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        // Không set loading = true khi thay đổi filter để tránh reload toàn trang
        // Loading chỉ được set = true lần đầu tiên (initial state)
  ...
  // Fetch card stats (không có date filter) - chỉ chạy 1 lần khi component mount
  useEffect(() => {
    const fetchCardStats = async () => {
      try {
        const statsResponse = await statisticService.getDashboardStats();
        if (statsResponse) {
          setCardStats(statsResponse);
        }
      } catch (err) {
  ...
  // Get summary stats from API (không có date filter, dữ liệu tổng)
  ...
  const cardSummary = cardStats?.summary || {};
  const totalRevenue = cardSummary.totalRevenue?.value || 0;
  const totalStudents = cardSummary.registeredStudents?.value || 0;
  const activeStudents = cardSummary.currentlyStudying?.value || 0;
  const completionRate = cardSummary.completionRate?.value || 0;

  // Get changes (already calculated from API, compared to previous month)
  const revenueChange = cardSummary.totalRevenue?.change || 0;
  const studentsChange = cardSummary.registeredStudents?.change || 0;
  const activeChange = cardSummary.currentlyStudying?.change || 0;
  const completionChange = cardSummary.completionRate?.change || 0;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20, marginBottom: 32 }}>
          ...
          >
            ...
            <div style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: 8, color: '#fff' }}>
              {cardSummary.totalRevenue?.formatted || formatCurrency(totalRevenue)}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: 8, color: '#fff' }}>
              {cardSummary.registeredStudents?.formatted || formatNumber(totalStudents)}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: 8, color: '#fff' }}>
              {cardSummary.currentlyStudying?.formatted || formatNumber(activeStudents)}
            </div>
          ...
          >
            ...
            <div style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: 8, color: '#fff' }}>
              {cardSummary.completionRate?.formatted || `${completionRate.toFixed(1)}%`}
            </div>
